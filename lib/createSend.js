...
module.exports = function (logger, targets, stateStore) {
  const q = async.queue(({target, data, stateID, to}, cb) => {
    ...
    console.log('before stateStore', stateID)
    stateStore.get(stateID, (err, state) => {
      console.log('from state store', err, state)
      if (err) {
      ...
      try {
        let nextState = _.cloneDeep(target(state.current, data))
        // if equal, dont store
        ...
        stateStore.set(stateID, state.currentDoc, nextState, data, (err) => {
